...
let
  ...
  pyqt6 =
    if isLinux then
      ...
      python3Packages.pyqt6.override {
        withLocation = false;
        withPdf = false;
        withPrintSupport = false;
        withSpeech = false;
        withWebSockets = false;
      };
  qasync =
    if isLinux then python3Packages.qasync else python3Packages.qasync.override { inherit pyqt6; };
in
python3Packages.buildPythonApplication (finalAttrs: {
  ...
  pypaBuildFlags = [
    ...
    "--config-setting=cmake.define.BILIHUD_LAYER_SHELL=${if isLinux then "ON" else "OFF"}"
  ];

  # Let the Python application wrapper carry Qt's plugin and platform paths.
  dontWrapQtApps = isLinux;
  makeWrapperArgs = lib.optionals isLinux [
    "\${qtWrapperArgs[@]}"
    "--prefix"
    "LD_LIBRARY_PATH"
    ":"
  ...
  nativeInstallCheckInputs = lib.optionals isLinux [ desktop-file-utils ];
  doInstallCheck = true;
  installCheckPhase = ''
