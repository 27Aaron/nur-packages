...
readonly release_api="https://api.github.com/repos/27Aaron/WeRead-Kit/releases/latest"
readonly package_file="pkgs/by-name/we/weread-kit/package.nix"
...
if [[ $current_version == "$latest_version" ]]; then
  echo "weread-kit is already up to date ($current_version)"
  exit 0
fi
...
if ! jq --exit-status --null-input \
  ...
  ' >/dev/null; then
  echo "Refusing to downgrade weread-kit from $current_version to $latest_version" >&2
  exit 1
fi

echo "Updating weread-kit from $current_version to $latest_version"
source_url="https://github.com/27Aaron/WeRead-Kit/archive/refs/tags/v${latest_version}.tar.gz"
...
echo "Updated weread-kit to $latest_version"
